...
def plot_all_linear_layer_weights_in_modulelist(model, epoch, epochs_to_plot):
    """
    Plot histograms of the weights from all nn.Linear layers in an nn.ModuleList named 'W' within the given PyTorch model
    only for specified epochs.

    Parameters:
    - model (nn.Module): The PyTorch model containing an nn.ModuleList 'W' with nn.Linear layers.
    - epoch (int): The current epoch number.
    - epochs_to_plot (list): List of epochs at which to plot the histograms.
    """
    # Check if the current epoch is in the list of epochs to plot
    if epoch in epochs_to_plot:
        # Access the nn.ModuleList 'W' in the model
        module_list = getattr(model, 'W', None)
        
        if module_list is None:
            raise ValueError("Model does not contain an attribute 'W' as an nn.ModuleList")
        
        # Iterate over all layers in the nn.ModuleList
        for i, layer in enumerate(module_list):
            if isinstance(layer, nn.Linear):
                ...
                plt.hist(weights.flatten(), bins=50, color='blue', edgecolor='black')
                plt.title(f'Epoch {epoch}: Histogram of Weight Values from Layer {i} in ModuleList W')
...
def plot_accuracy(accuracy_list, act_fn_name = None):
    """
