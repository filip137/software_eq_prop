...
class mlp_eqprop(nn.Module):
    def __init__(self, fcLayers, dt, T, Kmax, beta, loss, act_fn, data = None):
        ...
        self.device_pool = DevicePool("/home/filip/software_eq_prop/WeakResets/march_slope_x3_5k_5000_0.86_to_1.0.hdf5")
...
def train_network(net, train_loader, test_loader, optimizer, epochs):
    ...
    for rep in range(epochs):
        ...
        if (rep // 2) % 2 == 0:  # Train the second layer
...
def main():
    ...
    accuracy_list = train_network(net, train_loader, test_loader, optimizer = None, epochs = params['epochs'])   
